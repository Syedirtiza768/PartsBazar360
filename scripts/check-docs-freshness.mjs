...
function warnFallback(reason, files) {
  ...
  console.warn(
    `[docs-check] This commit touches ${files.length} file(s) under apps/*/src or packages/*/src ` +
      `without touching docs/. Check docs/Home.md and update the relevant note or docs/decisions.md if this is architecturally significant.`,
  );
...
let notes = [];
for (const dir of ["docs", "docs/apps", "docs/packages"]) {
  try {
...
const prompt =
  `You are a pre-commit check for a monorepo. Existing architecture docs (an Obsidian vault):\n${notes.join("\n")}\n\n` +
  `Staged diff (source files only, may be truncated):\n${diff}\n\n` +
  `Does this diff introduce a change significant enough that one of the docs notes above (or docs/decisions.md) ` +
