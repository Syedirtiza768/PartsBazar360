...
PART_ID=$($PSQL -t -A -c 'SELECT cp.id FROM "CanonicalPart" cp JOIN "SellerOffer" so ON so."canonicalPartId"=cp.id WHERE so.status='"'"'ACTIVE'"'"' AND cp."partClassKey" IS NOT NULL LIMIT 1;')
echo "part=$PART_ID"
test -n "$PART_ID"
...
s=d.get('shipping') or {}
print('shipping', {k:s.get(k) for k in ('state','weightKg','billableWeightKg','weightBasis','source','partClassKey','requiresFreightQuote')})
assert s.get('billableWeightKg'), 'PDP must resolve a class-default shipping weight'
assert s.get('source') in ('CLASS_DEFAULT','SPREADSHEET','SELLER','MEASURED','SIBLING_MPN','AI'), s
print('OK: three-state shipping contract live')
PY
...
curl -sL -o /dev/null -w "infographic=%{http_code} (404 expected until AI generates one)\n" "https://partsbazar360.com/api/search/parts/$PART_ID/infographic.svg"
echo DONE
