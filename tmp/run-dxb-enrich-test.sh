#!/bin/bash
# Run DXB OEM enrichment (PartSouq image CDN + febest.de OEM compatibility).
# Usage:
#   bash tmp/run-dxb-enrich-test.sh
#   DXB_LIMIT=500 bash tmp/run-dxb-enrich-test.sh
#   DXB_LIMIT=all bash tmp/run-dxb-enrich-test.sh   # full catalog
set -eu
cd /home/ubuntu/PartsBazar360

LIMIT="${DXB_LIMIT:-20}"
FORCE="${DXB_FORCE:-0}"
DELAY_MS="${DXB_DELAY_MS:-250}"
...
echo "Running DXB OEM enrich limit=${DXB_LIMIT:-all} force=$FORCE delayMs=$DELAY_MS"
