...
echo "==== Salvage listings: title language sample + counts ===="
docker exec partsbazar360-postgres-1 psql -U partsbazar_user -d partsbazar_db -c "
SELECT COUNT(*) AS total_salvage_active,
       COUNT(*) FILTER (WHERE p.title ~ '[A-Za-z]{4,}' AND (p.title ILIKE '% albero %' OR p.title ILIKE '% coperchio %' OR p.title ILIKE '% pompa %' OR p.title ILIKE '% altoparlante %' OR p.title ILIKE '% sterzo %' OR p.title ILIKE '% cinghia %' OR p.title ILIKE '% motore %' OR p.title ILIKE '% anteriore %' OR p.title ILIKE '% posteriore %' OR p.title ILIKE '% destra %' OR p.title ILIKE '% sinistra %')) AS italianish,
       COUNT(*) FILTER (WHERE p.title ILIKE '%gebraucht%' OR p.title ILIKE '% schaltung %' OR p.title ILIKE '% hinter %' OR p.title ILIKE '% vorder %' OR p.title ILIKE '% motor %' OR p.title ILIKE '% getriebe %') AS germanish
...
docker exec partsbazar360-postgres-1 psql -U partsbazar_user -d partsbazar_db -c "
SELECT p.title, p.brand, p.\"ebayItemId\", r.\"storeId\", r.title AS raw_title, r.\"listingStatus\", r.\"lastSyncedAt\"
FROM \"CanonicalPart\" p
JOIN \"SellerOffer\" o ON o.\"canonicalPartId\"=p.id JOIN \"Seller\" s ON s.id=o.\"sellerId\"
...
echo
echo "==== StoreId distribution for Salvage active parts (via RawStagingListing) ===="
docker exec partsbazar360-postgres-1 psql -U partsbazar_user -d partsbazar_db -c "
...
echo
echo "==== Offers missing price or currency or seller ===="
docker exec partsbazar360-postgres-1 psql -U partsbazar_user -d partsbazar_db -c "
SELECT s.name,
...
echo
echo "==== CanonicalParts with NO offers at all (stale) ===="
docker exec partsbazar360-postgres-1 psql -U partsbazar_user -d partsbazar_db -c "
...
echo
echo "==== OS doc count vs DB active parts ===="
...
SELECT COUNT(DISTINCT o.\"canonicalPartId\") FROM \"SellerOffer\" o JOIN \"Seller\" s ON s.id=o.\"sellerId\" WHERE o.status='ACTIVE' AND s.\"onboardingStatus\"='ACTIVE';"
