...
curl -sL -o /dev/null -w "buyer_checkout=%{http_code}\n" "$BASE/buyer/checkout/"
curl -sL -o /dev/null -w "api_health=%{http_code}\n" "$API/health"

echo "=== cart page markers ==="
curl -sL "$BASE/buyer/cart/" > /tmp/cart.html
python3 - <<'PY'
html=open('/tmp/cart.html',encoding='utf-8',errors='ignore').read()
markers=['Order summary','Ship to country','Your cart']
for m in markers:
    print(f"{m}={'YES' if m in html else 'NO'}")
# Next may hydrate client copy from JS chunks; ensure page loads.
assert 'Your cart' in html or 'Order summary' in html
print('CART_PAGE_OK')
PY

echo "=== login + quote checks ==="
...
OFFER_JSON=$(sudo docker compose exec -T postgres psql -U partsbazar_user -d partsbazar_db -t -A -c \
  "SELECT json_build_object('offerId', so.id)
   FROM \"SellerOffer\" so
...
curl -sL -o /tmp/add.json -X POST "$API/cart/$CART_ID/items" \
  -H "Content-Type: application/json" \
  -d "{\"offerId\":\"$OFFER_ID\",\"quantity\":1}"
ITEMS=$(python3 -c "import json; print(len(json.load(open('/tmp/add.json')).get('items') or []))")
...
bad=json.load(open('/tmp/q_bad.json'))
checks=[
  ...
  ('blank_rejected', bad.get('statusCode') == 400 or 'required' in str(bad.get('message','')).lower()),
]
print(json.dumps({
...
echo "=== buyer bundle marker ==="
sudo docker compose exec -T buyer-marketplace sh -c "grep -R \"Ship to country\" /app/apps/buyer-marketplace/.next/static -l | head -3"
echo DONE_VERIFY
