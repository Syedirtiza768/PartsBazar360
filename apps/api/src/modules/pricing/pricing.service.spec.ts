...
function serviceWithPolicy(policy: Partial<any> | null) {
  const pricingPolicy = policy
    ? {
        ...
        name: 'Test policy',
        version: 3,
        mode: 'COMMISSION_ON_SELLING_PRICE',
        percentRate: 0.3,
        fixedFee: 0,
        currency: 'USD',
        category: null,
...
describe('PricingService', () => {
  ...
  it('distinguishes a 30% markup from a 30% target margin', async () => {
    const markup = await serviceWithPolicy({ mode: 'COST_PLUS_MARKUP' }).quote(
      'seller-1',
      null,
      100,
    );
    ...
    const margin = await serviceWithPolicy({ mode: 'TARGET_MARGIN' }).quote(
      'seller-1',
      null,
      100,
    );
    expect(margin).toMatchObject({
      customerPrice: 142.86,
      marketplaceFee: 42.86,
      sellerProceeds: 100,
    });
  });

  it('applies fixed fees and caps without producing hidden negative proceeds', async () => {
    const quote = await serviceWithPolicy({
      mode: 'HYBRID_PERCENT_PLUS_FIXED',
      fixedFee: 5,
      maximumFee: 20,
    }).quote('seller-1', null, 100);
    expect(quote).toMatchObject({
      customerPrice: 100,
      marketplaceFee: 20,
      sellerProceeds: 80,
