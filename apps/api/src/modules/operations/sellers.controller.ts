  ...
  async updateStatus(
    ...
    if (!ONBOARDING_STATES.includes(body.status)) {
      throw new BadRequestException(
        `Unsupported onboarding status: ${body.status}`,
      );
    }
    ...
    const seller = await this.prisma.seller.findUnique({
      ...
      include: {
        profile: true,
        pricingAssignments: { where: { status: 'ACTIVE' } },
      },
    });
    ...
    if (body.status === 'ACTIVE') {
      if (complianceStatus !== 'VERIFIED')
        throw new BadRequestException(
          'Compliance must be VERIFIED before activation',
        );
      if (payoutStatus !== 'VERIFIED')
        throw new BadRequestException(
          'Payout account must be VERIFIED before activation',
        );
      if (seller.pricingAssignments.length === 0)
        throw new BadRequestException(
          'Assign an active pricing policy before activation',
        );
    }
    ...
    await this.prisma.$transaction(async (tx) => {
      if (
        seller.profile &&
        (body.complianceStatus || body.payoutStatus || body.status === 'ACTIVE')
      ) {
