...
export class PricingOperationsController {
  ...
  list() {
    return this.prisma.pricingPolicy.findMany({
      ...
      include: {
        _count: { select: { assignments: true, pricedOffers: true } },
      },
    });
  }
  ...
  async create(
    ...
    if (!PRICING_MODES.includes(body.mode as any))
      throw new BadRequestException(`Unsupported pricing mode: ${body.mode}`);
    const percentRate = Number(body.percentRate || 0);
    if (percentRate < 0 || percentRate >= 1)
      throw new BadRequestException('percentRate must be between 0 and 0.9999');

    const latest = await this.prisma.pricingPolicy.findFirst({
      where: { code },
    ...
    return this.prisma.pricingPolicy.create({
      data: {
        ...
        minimumPrice:
          body.minimumPrice === undefined ? null : Number(body.minimumPrice),
        maximumFee:
          body.maximumFee === undefined ? null : Number(body.maximumFee),
  ...
  async update(
    ...
    @Body()
    body: { status?: string; approvedBy?: string; effectiveTo?: string | null },
  ) {
  ...
  async assign(
    ...
    const policy = await this.prisma.pricingPolicy.findUnique({
      where: { id: body.pricingPolicyId },
    });
    if (!policy || policy.status !== 'ACTIVE')
      throw new BadRequestException('Pricing policy must be ACTIVE');
