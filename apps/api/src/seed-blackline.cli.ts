/**
 * Sync Blackline Auto Parts from RealTrack.
 * English titles + upsert dedupe (ebayItemId / externalOfferId / sourceKey).
 *
 *   node dist/src/seed-blackline.cli.js
 *   SEED_LISTING_LIMIT=50 node dist/src/seed-blackline.cli.js
 */
...
async function main() {
  ...
  const store = MARKETPLACE_SELLERS.blackline;
  const report: any = {
  ...
  try {
    ...
    const reportPath = path.resolve(
      process.env.SEED_REPORT_PATH || '/tmp/seed-blackline-report.json',
    );
