/**
 * FEBI DB-only MVL compatibility backfill: copy only compatibility already verified against MVL
  ...
  while (true) {
    const rows = await prisma.$queryRawUnsafe(`
      WITH candidates AS (
        ...
        WHERE u.id > $1
          AND upper(trim(u.brand)) = 'FEBI'
          AND EXISTS (SELECT 1 FROM "SellerOffer" active_so WHERE active_so."canonicalPartId" = u.id AND active_so."sellerId" = 'seller-superior-auto-parts' AND active_so.status = 'ACTIVE')
