...
export type StagedFilterController = {
  applied: FilterState;
  appliedKey: string;
  pending: FilterState;
  pendingKey: string;
...
export function useStagedFilters({
  ...
  const [previewResultCount, setPreviewResultCount] = useState<
    number | undefined
  >(resultCount);
  ...
  const priceError = priceRangeError(pending);

  useEffect(() => {
    if (!isDirty || priceError) {
    ...
    const timer = window.setTimeout(async () => {
      try {
        const href = filterStateToHref(params, pending, path);
        ...
        const response = await fetch(
          `${API_BASE_URL}/search/parts?${search.toString()}`,
          {
            cache: "no-store",
        ...
        if (!response.ok)
          throw new Error(`Count preview failed (${response.status})`);
  ...
  }, [appliedKey, isDirty, params, path, pending, pendingKey, priceError, resultCount]);
  ...
  const clearGroup = (groupId: FilterGroupId) => {
    setPending((prev) => {
      if (groupId === "category")
        return { ...prev, category: [], categoryGroup: [] };
      if (groupId === "price") return { ...prev, minPrice: "", maxPrice: "" };
      if (groupId === "interchange")
        return { ...prev, includeInterchange: true };
      return { ...prev, [groupId]: [] };
    });
  };
  ...
  const toggleInterchange = () => {
    setPending((prev) => ({
      ...prev,
      includeInterchange: !prev.includeInterchange,
    }));
  };

  const apply = () => {
    if (priceError) return;
    router.push(filterStateToHref(params, pending, path));
    onApply?.();
  };
  ...
  if (disabled) {
    return (
      ...
      >
        <span
          aria-hidden="true"
          className="h-4 w-4 shrink-0 rounded border border-slate-200 bg-slate-50"
        />
        <span className="min-w-0 flex-1 truncate">{label}</span>
        {count != null && (
          <span className="shrink-0 font-mono text-xs tabular-nums">0</span>
        )}
      </span>
    );
  }
    ...
    <section className="border-b border-slate-200 pb-2 last:border-0">
      <div className="flex items-center gap-1">
        <button
          ...
          onClick={onToggle}
          aria-expanded={open}
          className="flex min-h-touch min-w-0 flex-1 items-center justify-between gap-2 py-2.5 text-left lg:min-h-0"
        >
          <span className="min-w-0">
            ...
            <span className="mt-0.5 block truncate text-xs font-medium text-graphite-600">
              {summary}
            </span>
          </span>
          <ChevronRightIcon
        ...
        {selectedCount > 0 && (
          <button
            type="button"
            onClick={onClear}
            className="shrink-0 rounded px-1.5 py-1 text-[11px] font-semibold text-brand-700 hover:bg-brand-50"
          >
            Clear
          </button>
        )}
...
function FacetOptions({
  ...
  if (options.length === 0) {
    return (
      <p className="px-2.5 py-2 text-sm text-graphite-600">
...
function CategoryOptions({
  ...
  const sorted = categoryGroups
    .filter((group) => {
      ...
      if (!name) return false;
      if (group.count > 0)
        return !/^general$/i.test(name) || selectedGroups.has(group.name);
      return (
        selectedGroups.has(group.name) ||
        group.categories.some((cat) => selectedCats.has(cat.name))
      );
    })
    .sort((a, b) => {
      const aSelected =
        selectedGroups.has(a.name) ||
        a.categories.some((cat) => selectedCats.has(cat.name));
      const bSelected =
        selectedGroups.has(b.name) ||
        b.categories.some((cat) => selectedCats.has(cat.name));
      if (aSelected !== bSelected) return aSelected ? -1 : 1;
    ...
    <>
      <div className="space-y-1">
        {visible.map((group) => {
          ...
          const hasSelectedChild = group.categories.some((cat) =>
            selectedCats.has(cat.name),
          );
          const cats = group.categories
            .filter((cat) => {
              const name = cat.name.trim();
              if (!name || /^general$/i.test(name))
                return selectedCats.has(cat.name);
                  ...
                  <div className="ml-2.5 space-y-0.5 border-l border-slate-100 pl-2.5">
                    {cats.map((cat) => (
                      <FilterOption
                        ...
                        disabled={
                          cat.count === 0 && !selectedCats.has(cat.name)
                        }
                        label={cat.name}
                        count={cat.count}
  ...
  return (
    <div className="grid grid-cols-2 gap-2 px-2.5 py-1.5">
      <label className="min-w-0">
        <span className="mb-1 block text-[11px] font-semibold uppercase text-graphite-500">
          Min
        </span>
        <input
          inputMode="decimal"
          value={minPrice}
          onChange={(event) =>
            onChange({ minPrice: normalizePriceInput(event.target.value) })
          }
      ...
      <label className="min-w-0">
        ...
        <input
          ...
          value={maxPrice}
          onChange={(event) =>
            onChange({ maxPrice: normalizePriceInput(event.target.value) })
          }
          className="h-10 w-full rounded-md border border-slate-300 px-2 text-sm font-semibold text-slate-800 outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/30"
          placeholder="Any"
        />
      ...
      {error && (
        <p className="col-span-2 px-0.5 text-xs font-medium text-red-700" role="alert">
          {error}
        </p>
      )}
    </div>
...
function FilterGroupBody({
  ...
  if (groupId === "price") {
    return (
      <PriceOptions
        ...
        onChange={controller.updatePrice}
      />
    );
  }
  ...
  if (groupId === "interchange") {
    return (
      <InterchangeOptions
        includeInterchange={pending.includeInterchange}
        onToggle={controller.toggleInterchange}
      />
    );
  }

  const facetMap: Record<
    Exclude<FilterGroupId, "category" | "price" | "interchange">,
    Facet[]
  > = {
      ...
      <div className="min-w-0">
        <p className="text-sm font-semibold text-slate-900">
          {controller.pendingCount > 0
            ? `${controller.pendingCount} selected`
            : "No filters selected"}
        </p>
        {controller.priceError && (
          <p className="text-xs font-medium text-red-700">Fix the price range.</p>
        )}
        {controller.isDirty && (
          <p className="text-xs text-graphite-600">Changes are staged.</p>
        )}
      </div>
...
export function FilterSectionsClient({
  ...
  // Desktop quick filters and the full filter sheet intentionally share one
  // staged controller. Moving between them must never discard selections that
  // have not been applied yet.
  const localController = useStagedFilters({ params, resultCount, onApply, path });
  const controller = sharedController ?? localController;
  const groups = FILTER_GROUPS.filter((group) =>
    groupAvailable(group.id, facets, params),
  );
  const selectedGroup = groups.find(
    (group) => groupSelectedCount(group.id, controller.pending) > 0,
  )?.id;
  const [openGroup, setOpenGroup] = useState<FilterGroupId | null>(
    selectedGroup ?? groups[0]?.id ?? "price",
  );

  useEffect(() => {
    if (openGroup && !groups.some((group) => group.id === openGroup)) {
    ...
    <div className="space-y-1">
      ...
      {variant === "advanced" && (
        <div className="mb-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-graphite-700">
          All available filters use the same staged selections as the quick
          filters and sidebar.
        </div>
      )}
...
export function QuickFilterRow({
  ...
  const groups = FILTER_GROUPS.filter(
    (group) => group.quick && groupAvailable(group.id, facets, params),
  ).slice(0, 6);
  ...
  return (
    <>
      ...
      >
        {groups.map((group) => {
          const selectedCount = groupSelectedCount(
            group.id,
            controller.pending,
          );
    ...
    <div>
      ...
      <div className="mb-3 flex items-start justify-between gap-3 border-b border-slate-200 pb-3">
        <div className="min-w-0">
          ...
          <p className="truncate text-sm text-graphite-600">
            {groupSelectionSummary(groupId, controller.pending)}
          </p>
        </div>
