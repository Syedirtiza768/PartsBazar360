...
/**
 * Category landing page — page 2 and beyond.
 *
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return taxonomyMetadata({ kind: "category", slug, page: parsePageParam(page) });
}

export default async function Page({ params }: PageProps) {
  const { slug, page } = await params;
  return renderTaxonomy({ kind: "category", slug, page: parsePageParam(page) });
}
