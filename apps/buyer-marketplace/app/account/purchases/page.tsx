...
type ServerOrder = {
  ...
  sellerOrders: Array<
    StoredOrder["sellerOrders"][number] & { items: StoredOrder["items"] }
  >;
};

export default function PurchasesPage() {
  const [orders, setOrders] = useState<StoredOrder[] | null>(null);
  const { isAuthenticated, authHeaders, ready } = useAuth();
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const serverOrders = (await response.json()) as ServerOrder[];
        setOrders(
          serverOrders.map((order) => ({
            ...
            items: order.sellerOrders.flatMap(
              (sellerOrder) => sellerOrder.items,
            ),
  ...
  }, [authHeaders, isAuthenticated, ready]);

  return (
    <section>
      <div className="flex flex-wrap items-end justify-between gap-2 border-b-2 border-graphite-950 pb-3">
        <div className="min-w-0">
          <p className="eyebrow">Purchase history</p>
          <h2 className="mt-1 font-display text-2xl font-black uppercase text-slate-950 sm:text-3xl">
            Orders & tracking
          </h2>
        </div>
        {orders && (
          <span className="font-mono text-xs font-bold text-graphite-600">
      ...
      {orders?.length === 0 ? (
        <div className="mt-6 border-2 border-slate-950 bg-white">
          <EmptyState
            variant="page"
            icon={<ReceiptIcon />}
            title="No purchases on this device"
            description="Completed checkouts will appear here with seller shipment and support actions."
          >
          ...
          {(orders || []).map((order) => (
            ...
            >
              <header className="flex flex-wrap items-center justify-between gap-3 border-b border-stone-300 bg-canvas-sunk px-4 py-3 sm:px-5">
                ...
                <span className="border border-brand-300 bg-brand-50 px-2.5 py-1 text-xs font-black uppercase tracking-wide text-brand-800">
              ...
              <div className="grid gap-5 p-4 sm:p-5 lg:grid-cols-[1fr_auto]">
                <div>
                  <p className="flex items-center gap-2 text-sm font-bold text-slate-950">
                    <PackageIcon className="h-4 w-4 text-brand-700" />
                    {order.items.reduce(
                      (sum, item) => sum + item.quantity,
                      0,
                    )}{" "}
                    item{order.items.length === 1 ? "" : "s"} across{" "}
                ...
                <div className="flex flex-wrap items-end justify-between gap-x-6 gap-y-3 border-t border-stone-200 pt-4 lg:block lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0">
                  <p className="price text-xl">
                    {formatPrice(order.totalAmount, order.currency)}
                  </p>
                  <Link
                    href={`/account/purchases/${encodeURIComponent(order.id)}`}
                    className="inline-flex min-h-touch items-center gap-2 text-sm font-black uppercase tracking-wide text-brand-700 lg:mt-3"
                  >
                    Order details <ArrowRightIcon className="h-4 w-4" />
                  </Link>
                </div>
              </div>
            </article>
