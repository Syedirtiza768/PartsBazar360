...
export default function PurchasesPage() {
  ...
  return (
    <section>
      <div className="flex flex-wrap items-end justify-between gap-2 border-b-2 border-graphite-950 pb-3"><div className="min-w-0"><p className="eyebrow">Purchase history</p><h2 className="mt-1 font-display text-2xl font-black uppercase text-slate-950 sm:text-3xl">Orders & tracking</h2></div>{orders && <span className="font-mono text-xs font-bold text-graphite-600">{orders.length} orders</span>}</div>
      {orders?.length === 0 ? (
        ...
        <div className="mt-5 space-y-4">
          {(orders || []).map((order) => (
            <article key={order.id} className="border border-stone-300 bg-white">
              <header className="flex flex-wrap items-center justify-between gap-3 border-b border-stone-300 bg-canvas-sunk px-4 py-3 sm:px-5"><div className="min-w-0"><p className="part-number break-anywhere font-bold text-graphite-950">Order {order.id}</p><p className="mt-0.5 text-xs text-graphite-600">Placed {new Date(order.createdAt).toLocaleDateString()}</p></div><span className="border border-brand-300 bg-brand-50 px-2.5 py-1 text-xs font-black uppercase tracking-wide text-brand-800">{order.paymentStatus.replace(/_/g, " ")}</span></header>
              <div className="grid gap-5 p-4 sm:p-5 lg:grid-cols-[1fr_auto]">
                ...
                <div className="flex flex-wrap items-end justify-between gap-x-6 gap-y-3 border-t border-stone-200 pt-4 lg:block lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0"><p className="price text-xl">{formatPrice(order.totalAmount, order.currency)}</p><Link href={`/account/purchases/${encodeURIComponent(order.id)}`} className="inline-flex min-h-touch items-center gap-2 text-sm font-black uppercase tracking-wide text-brand-700 lg:mt-3">Order details <ArrowRightIcon className="h-4 w-4" /></Link></div>
              </div>
            </article>
          ))}
        </div>
