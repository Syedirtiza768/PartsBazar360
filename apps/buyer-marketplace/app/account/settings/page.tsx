...
export default function SettingsPage() {
  ...
  return (
    <section>
      <div className="border-b-2 border-graphite-950 pb-3"><p className="eyebrow">Buyer preferences</p><h2 className="mt-1 font-display text-2xl font-black uppercase text-slate-950 sm:text-3xl">Settings</h2></div>
      <form onSubmit={save} className="mt-6 max-w-2xl space-y-6">
        <section className="border border-stone-300 bg-white p-4 sm:p-6"><h3 className="font-display text-lg font-black uppercase text-slate-950">Contact details</h3><p className="mt-1 text-sm text-graphite-600">Used to prefill future buyer interactions on this device.</p><div className="mt-5 grid gap-4 sm:grid-cols-2"><Input label="Full name" value={name} onChange={(event) => setName(event.target.value)} autoComplete="name" /><Input label="Email address" value={email} onChange={(event) => setEmail(event.target.value)} type="email" inputMode="email" autoComplete="email" autoCapitalize="none" autoCorrect="off" spellCheck={false} /></div></section>
        <section className="border border-stone-300 bg-white p-4 sm:p-6"><h3 className="font-display text-lg font-black uppercase text-slate-950">Notifications</h3><div className="mt-4 space-y-4"><Checkbox checked={fitmentAlerts} onChange={(event) => setFitmentAlerts(event.target.checked)} label="Fitment and listing updates" description="Compatibility clarification, watched listing changes, and seller replies." /><Checkbox checked={orderAlerts} onChange={(event) => setOrderAlerts(event.target.checked)} label="Order and return updates" description="Payment, dispatch, tracking, delivery, returns, and refund status." /></div></section>
