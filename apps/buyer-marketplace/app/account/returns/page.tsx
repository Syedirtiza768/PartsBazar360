...
export default function ReturnsPage() {
  ...
  return (
    <section>
      <div className="border-b-2 border-graphite-950 pb-3"><p className="eyebrow">Resolution centre</p><h2 className="mt-1 font-display text-2xl font-black uppercase text-slate-950 sm:text-3xl">Returns, refunds & issues</h2><p className="mt-2 max-w-2xl text-pretty text-sm text-graphite-700">Start with the seller and the exact order. If the issue cannot be resolved, the same case can be reviewed by marketplace support.</p></div>
      ...
      <div className="mt-8">
        ...
        {orders.length ? <div className="mt-3 space-y-3">{orders.map((order) => { const displayOrderNumber = order.orderNumber || order.id; return <div key={order.id} className="flex flex-col gap-3 border border-stone-300 bg-white p-4 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between sm:gap-4"><div className="min-w-0"><p className="part-number break-anywhere font-bold text-graphite-950">{displayOrderNumber}</p><p className="mt-1 text-xs text-graphite-600">{order.items.length} line item{order.items.length === 1 ? "" : "s"} · {new Date(order.createdAt).toLocaleDateString()}</p></div><Link href={`/support?orderId=${encodeURIComponent(order.id)}&category=RETURNS&subject=${encodeURIComponent(`Return or issue for order ${displayOrderNumber}`)}`} className={buttonClasses({ variant: "outline" })}>Open request <ArrowRightIcon className="h-4 w-4" /></Link></div>; })}</div> : <div className="mt-3 border border-stone-300 bg-white p-5"><p className="flex items-center gap-2 text-sm font-bold text-slate-950"><CheckCircleIcon className="h-4 w-4 text-brand-700" />No purchases available on this device</p><p className="mt-1 text-sm text-slate-600">If you checked out elsewhere, open a general support request and include the order number from your confirmation.</p><Link href="/support?category=RETURNS" className={`${buttonClasses({ variant: "outline" })} mt-4`}>Contact support</Link></div>}
      </div>
    </section>
  );
}
